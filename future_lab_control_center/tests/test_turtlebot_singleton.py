...
from backend.ros2_nodes import cobot_node as cobot_module
...
class CobotSingletonTest(unittest.TestCase):
    def setUp(self):
        self.previous_node = cobot_module._cobot_node
        cobot_module._cobot_node = None

    def tearDown(self):
        cobot_module._cobot_node = self.previous_node
    ...
    def test_concurrent_requests_construct_exactly_one_node(self):
        ...
        def get_node():
            start.wait()
            instances.append(cobot_module.get_cobot_node())

        with patch.object(cobot_module, "CobotNode", FakeCobotNode):
            threads = [threading.Thread(target=get_node) for _ in range(workers)]
            for thread in threads:
                thread.start()
            for thread in threads:
